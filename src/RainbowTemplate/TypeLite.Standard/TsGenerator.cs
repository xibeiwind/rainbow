    ...
    public class TsGenerator
    {
        ...
        public TsGenerator()
        {
            ...
            _typeFormatters.RegisterTypeFormatter<TsClass>((type, formatter) =>
            {
                ...
                return tsClass.Name + "<" + string.Join(", ",
                           tsClass.GenericArguments.Select(a =>
                               a as TsCollection != null
                                   ? GetFullyQualifiedTypeName(a) + "[]"
                                   : GetFullyQualifiedTypeName(a))) + ">";
            });
            _typeFormatters.RegisterTypeFormatter<TsSystemType>((type, formatter) =>
                ((TsSystemType) type).Kind.ToTypeScriptString());
            _typeFormatters.RegisterTypeFormatter<TsCollection>((type, formatter) =>
            {
                ...
                if (itemTypeAsClass == null || !itemTypeAsClass.GenericArguments.Any()) return GetTypeName(itemType);
        ...
        /// </summary>
        public ReadOnlyDictionary.IReadOnlyDictionary<Type, TsTypeFormatter> Formaters =>
            new ReadOnlyDictionaryWrapper<Type, TsTypeFormatter>(_typeFormatters._formatters);
        ...
        public string DefaultMemberTypeFormatter(TsProperty tsProperty, string memberTypeName)
        {
            ...
            return memberTypeName +
                   (isCollection ? string.Concat(Enumerable.Repeat("[]", asCollection.Dimension)) : "");
        }
        ...
        public string Generate(TsModel model, TsGeneratorOutput generatorOutput)
        {
                ...
                || (generatorOutput & TsGeneratorOutput.Fields) == TsGeneratorOutput.Fields)
            {
                ...
                foreach (var reference in _references.Concat(model.References)) AppendReference(reference, sb);
                sb.AppendLine();
            }
        ...
        {
            var classes = module.Classes.Where(c => !_typeConvertors.IsConvertorRegistered(c.Type) && !c.IsIgnored)
                .OrderBy(c => GetTypeName(c)).ToList();
            var enums = module.Enums.Where(e => !_typeConvertors.IsConvertorRegistered(e.Type) && !e.IsIgnored)
                .OrderBy(e => GetTypeName(e)).ToList();
            ...
            if (generateModuleHeader) sb.AppendLine("}");
        }
        ...
        protected virtual void AppendClassDefinition(TsClass classModel, ScriptBuilder sb,
            TsGeneratorOutput generatorOutput)
        {
            ...
            using (sb.IncreaseIndentation())
            {
                foreach (var property in members.Where(p => !p.IsIgnored).OrderBy(p => GetPropertyName(p)))
                {
        ...
        {
            string typeName = GetTypeName(enumModel);
            string visibility =
                (output & TsGeneratorOutput.Enums) == TsGeneratorOutput.Enums ||
                (output & TsGeneratorOutput.Constants) == TsGeneratorOutput.Constants
                    ? "export "
                    : "";

            _docAppender.AppendEnumDoc(sb, enumModel, typeName);

            string constSpecifier = GenerateConstEnums ? "const " : string.Empty;
        ...
        {
            if (!classModel.Constants.Any()) return;
            ...
            using (sb.IncreaseIndentation())
            {
                foreach (var property in classModel.Constants)
                {
                    ...
                    _docAppender.AppendConstantModuleDoc(sb, property, GetPropertyName(property),
                        GetPropertyType(property));
                    sb.AppendFormatIndented("export const {0}: {1} = {2};", GetPropertyName(property),
                        GetPropertyType(property), GetPropertyConstantValue(property));
                    sb.AppendLine();
                }
            }
        ...
        public string GetFullyQualifiedTypeName(TsType type)
        {
            ...
            if (type.Type.IsGenericParameter) return GetTypeName(type);
        ...
        public string GetCollectionModuleName(TsCollection collectionType, string moduleName)
        {
            if (collectionType.ItemsType as TsModuleMember != null &&
                !_typeConvertors.IsConvertorRegistered(collectionType.ItemsType.Type))
                if (!collectionType.ItemsType.Type.IsGenericParameter)
                    moduleName = ((TsModuleMember) collectionType.ItemsType).Module != null
                        ? GetModuleName(((TsModuleMember) collectionType.ItemsType).Module)
                        : string.Empty;
        ...
        {
            if (_typeConvertors.IsConvertorRegistered(type.Type)) return _typeConvertors.ConvertType(type.Type);
